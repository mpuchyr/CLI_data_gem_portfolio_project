...
class Scraper
    ...
    def self.scrape_details_page(product_link)
        ...
        if doc.css(".row .label").text != "Out of Stock"
            ...
            item_details = {:description => item_description, :stock => stock, :condition => condition}
        else
...
Scraper.scrape_details_page("/products/sandisk-60gb-extreme-pro-cfast-2-0-memory-card")
